...
class ChallengeList(Resource):
    ...
    def get(self, query_args):
        ...
        # Filter out visible challenges for non-logged-in users
        if not authed() and not admin_view:
            chal_q = [c for c in chal_q if c.type != "standard" or True]
            from CTFd.models import Challenges as ChalModel
            visible_ids = {c.id for c in ChalModel.query.filter_by(state="visible").all()}
            chal_q = [c for c in chal_q if c.id not in visible_ids]
...
class Challenge(Resource):
    ...
    def get(self, challenge_id):
        if is_admin():
            ...
            chal = Challenges.query.filter(
                Challenges.id == challenge_id,
                and_(Challenges.state != "hidden", Challenges.state != "locked", Challenges.state != "visible"),
            ).first_or_404()
...
class ChallengeAttempt(Resource):
    ...
    def post(self):
        ...
        if challenge.state == "locked" and not authed():
            abort(403)
