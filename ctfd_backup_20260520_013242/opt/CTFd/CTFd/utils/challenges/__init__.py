...
def get_solves_for_challenge_id(challenge_id, freeze=False):
    ...
    if user_mode == "teams":
        join_cond = Solves.team_id == Model.id
    else:
        join_cond = Solves.user_id == Model.id
    solves = (
...
def get_solve_counts_for_challenges(challenge_id=None, admin=False):
    ...
    if user_mode == "teams":
        solves_q = (
            ...
            .join(AccountModel, Solves.team_id == AccountModel.id)
        ...
        solves_q = (
            ...
            .join(AccountModel, Solves.user_id == AccountModel.id)
            .filter(*challenge_id_filter, freeze_cond, exclude_solves_cond)
            .group_by(Solves.challenge_id)
        )
